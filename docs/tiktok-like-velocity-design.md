...
## 1. 두 가지 모드는 난이도가 완전히 다르다

| | 모드 A: 특정 유저 | 모드 B: 전체 컨텐츠 |
|---|---|---|
| 대상 집합 | 명확 (추적 계정 목록) | 불명확 — **먼저 정의해야 함** |
| 발굴(discovery) | 불필요 | 필수 (가장 어려운 부분) |
| 폴링 비용 | 계정 수에 비례 | 탐색 범위에 비례 (폭발적) |
| 공식 API로 가능? | ✅ 현실적 | ⚠️ 쿼터상 부분적 |
| 기준선(baseline) | 그 계정의 과거 영상 | 같은 나이대 코호트 |
