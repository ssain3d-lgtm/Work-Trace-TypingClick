...
| **Research API** `POST /v2/research/video/query/` | 공개 영상 전반. `create_date`·`username`·`hashtag_name`·`keyword`·`region_code` 필터, boolean(AND/OR/NOT) 조합 | 정확한 정수 | 무료. **1,000 req/day · 100,000 records/day**, 1콜당 최대 100건, 00:00 UTC 리셋 | ⚠️ **미국·유럽 비영리 학술기관 소속만** + 수동 심사 — **한국 거주자는 대상 아님** | A + B |
...
> 🇰🇷 **한국 거주자에게 Research API는 사실상 닫혀 있다.** 2023년 미국·유럽 학술 연구자를 대상으로 개시된 이후 한국은 자격 지역에 포함된 적이 없다. 따라서 한국에서 현실적인 선택지는 **Display API(본인 계정만) · 서드파티 유료 API · [모드 C(내 피드 캡처)](./tiktok-fyp-capture-design.md)** 셋뿐이다. §5의 쿼터 계산은 참고용으로만 읽을 것.
...
- **공식 API 우선.** Research API는 미국·유럽 비영리 학술기관 소속 + 수동 심사가 필요하다 — **한국 거주자는 자격 대상이 아니며**, 상업적 용도로는 더더욱 어렵다.
- **한국법 관점:** 개인이 자기 기기에서 로컬로 수집·분석하는 것은 실무상 위험이 낮다. 다만 이를 **서비스화하거나 수집 데이터를 재배포**하는 순간 저작권법상 **데이터베이스제작자의 권리** 침해와 **개인정보보호법**(크리에이터 식별정보의 대량 처리) 쟁점이 함께 생긴다. 그 단계에서 반드시 법률 검토를 받을 것.
