...
| 네이티브 앱 트래픽 MITM (루팅) | ✅ | ✅ | ❌ **Knox 영구 손상 + 한국 내수 모델은 언락 경로 자체가 막힘** | ❌ |
...
- **One UI 8 / Android 16 이후 개발자 옵션에서 `OEM 잠금해제` 항목 자체가 사라지는 방향으로 정리됐다.** 업데이트 과정에서 OEM Status가 Unlocked → Locked로 되돌아간 사례도 보고된다.
- 지역별로 정책이 다르며, **미국·캐나다 통신사 모델은 애초에 토글이 없다.**

**🇰🇷 한국 내수 모델(`N` 접미사, 예: `SM-S928N`)이라면:** 한국 커뮤니티에서는 오래전부터 내수 모델의 부트로더 언락이 막혀 있는 것으로 통한다. 다만 이 문서를 쓰면서 공식 문서로 확정하지는 못했다 — **30초면 직접 확인할 수 있다:**

> 설정 → 휴대전화 정보 → 소프트웨어 정보 → 빌드번호 7회 탭 → 설정 → 개발자 옵션 → **`OEM 잠금해제` 항목이 보이는가?**

항목이 없으면 그것으로 끝이다. **있더라도 §2가 같은 결과를 대가 없이 주므로 굳이 확인할 실익은 없다.** 결론은 어느 쪽이든 바뀌지 않는다.
...
- **🇰🇷 한국어 UI는 반올림이 영어권보다 10배 거칠다.** 영어 `12.3K`는 눈금이 100 단위지만, 한국어 `1.2만`은 **1,000 단위**다(12,000 → 13,000). 바이럴 컨텐츠가 몰리는 1만~100만 구간이 정확히 이 구간이라, 한국어 화면에서 카운터를 읽어 Δ를 만들려는 시도는 **완전히 무의미하다.** OCR도 마찬가지 — 문제는 인식률이 아니라 원본 표시값 자체다
...
- **네이티브 앱 트래픽 가로채기는 포기한다.** 비루팅은 APK 재서명이 필요해 계정이 위험하고, 루팅은 갤럭시에서 Knox e-fuse가 영구 트립된다. 한국 내수 모델은 언락 경로 자체가 막혀 있을 가능성이 높고, One UI 8 이후로는 토글이 사라지는 방향이다.
