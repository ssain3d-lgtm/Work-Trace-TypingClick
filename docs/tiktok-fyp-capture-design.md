...
> 🇰🇷 **한국 사용자에게는 순수한 이득이다.** 내 FYP는 이미 한국 로케일로 개인화돼 있으므로 **지역 필터링이 공짜로 따라온다.** Research API였다면 `region_code=KR` 필터를 걸어야 했겠지만 ([한국은 애초에 자격 지역이 아니다](./tiktok-like-velocity-design.md#2-데이터-소스-비교)), 모드 C는 아무것도 안 해도 국내 트렌드를 잡는다.
